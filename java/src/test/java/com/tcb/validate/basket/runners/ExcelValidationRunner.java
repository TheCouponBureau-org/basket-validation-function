...
import com.tcb.validate.basket.core.BasketValidator;
...
public class ExcelValidationRunner {
	...
	public ValidationSummary runValidation() {
		...
		String filePath = System.getProperty("user.dir") + "/POS Basket Validation Use Cases.xlsx";
		...
		try {
			...
			Sheet sheet = workbook.getSheet("Use Cases");

			if (sheet == null) {
				throw new RuntimeException("Sheet 'Use Cases' not found!");
			}
			...
			for (int i = 1; i <= sheet.getLastRowNum(); i++) {
				...
				try {
					...
					if (inputCell == null || expectedCell == null || inputCell.getCellType() == CellType.BLANK
